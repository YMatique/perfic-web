...
<div class="p-4 lg:p-6">
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-700">
        <div class="p-4 lg:p-6 border-b border-gray-200 dark:border-zinc-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-zinc-100">
                Lista de Transações ({{ $transactions ? $transactions->total() : 0 }})
            </h3>
        </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-4">
                                @if($transaction->category)
                                    <div class="w-10 h-10 rounded-lg flex items-center justify-center" 
                                        ...
                                        <span class="material-icons text-base">{{ $transaction->category->icon }}</span>
                                    </div>
                                @else
                                    <div class="w-10 h-10 rounded-lg flex items-center justify-center bg-gray-100 dark:bg-zinc-700">
                                        <span class="material-icons text-base text-gray-500">category</span>
                                    </div>
                                @endif
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="text-right">
                                    <p class="text-sm font-medium {{ $transaction->type === 'income' ? 'text-success-600 dark:text-success-400' : 'text-danger-600 dark:text-danger-400' }}">
                                        {{ $transaction->type === 'income' ? '+' : '-' }}MZN {{ number_format($transaction->amount, 2, ',', '.') }}
                                    </p>
    ...
    @if($showForm)
        <div class="fixed inset-0 z-50 bg-black/30 dark:bg-black/50 backdrop-blur-sm overflow-y-auto h-full w-full">
            <div class="relative top-10 mx-auto p-6 border w-96 shadow-2xl rounded-xl bg-white/95 dark:bg-zinc-800/95 backdrop-blur-md border-gray-200/50 dark:border-zinc-700/50">
                <div class="mt-3">
                    ...
                    <form wire:submit="{{ $editingTransaction ? 'update' : 'store' }}" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-2">Localização (Opcional)</label>
                            <input type="text" 
                                   wire:model="location" 
                                   class="w-full border border-gray-200 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-gray-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 px-3 py-2.5 transition-colors text-sm" 
