    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p
                    class="text-2xl font-bold {{ $reportData['summary']['net_balance'] >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                    MZN {{ number_format($reportData['summary']['net_balance'], 2, ',', '.') }}
                </p>
            </div>
            <div
                class="w-12 h-12 {{ $reportData['summary']['net_balance'] >= 0 ? 'bg-green-100 dark:bg-green-900' : 'bg-red-100 dark:bg-red-900' }} rounded-lg flex items-center justify-center">
                <span
                    class="material-icons {{ $reportData['summary']['net_balance'] >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
        ...
        <div class="space-y-3">
            <div class="flex justify-between">
                ...
                <span
                    class="text-sm font-medium text-gray-900 dark:text-zinc-100">{{ $reportData['summary']['transactions_count'] }}</span>
            </div>
            <div class="flex justify-between">
            ...
            <div class="flex justify-between">
                ...
                <span class="text-sm font-medium text-gray-900 dark:text-zinc-100">
                    {{ number_format($reportData['period']['days'] ,0)}} dias
                </span>
            </div>
        </div>
    </div>
    ...
    @if (!empty($reportData['category_breakdown']['income']) && count($reportData['category_breakdown']['income']) > 0)
        <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <span
                    class="text-xs text-gray-500 dark:text-zinc-400">{{ count($reportData['category_breakdown']['income']) }}
                    categorias</span>
            </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-zinc-400">{{ $category['count'] }} transações
                                </p>
                            </div>
                        </div>
                        <div class="text-right">
                            ...
                            <p class="text-xs text-gray-500 dark:text-zinc-400">
                                {{ number_format($category['percentage'], 1) }}%</p>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    @if (!empty($reportData['category_breakdown']['expense']) && count($reportData['category_breakdown']['expense']) > 0)
        <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <span
                    class="text-xs text-gray-500 dark:text-zinc-400">{{ count($reportData['category_breakdown']['expense']) }}
                    categorias</span>
            </div>
...
@if (!empty($reportData['top_transactions']) && count($reportData['top_transactions']) > 0)
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="text-xs text-gray-500 dark:text-zinc-400">Top
                {{ count($reportData['top_transactions']) }}</span>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-zinc-700">
                        <th class="text-left py-3 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase">
                            Descrição</th>
                        <th class="text-left py-3 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase">
                            Categoria</th>
                        <th class="text-left py-3 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase">Data
                        </th>
                        <th class="text-right py-3 text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase">Valor
                        </th>
                    </tr>
                </thead>
                        ...
                        <tr>
                            ...
                            <td class="py-3">
                                <div class="flex items-center space-x-2">
                                    <span
                                        class="material-icons text-sm text-gray-400">{{ $transaction['category_icon'] }}</span>
                                    <span
                                        class="text-sm text-gray-600 dark:text-zinc-400">{{ $transaction['category_name'] }}</span>
                                </div>
                            </td>
                            <td class="py-3">
                            ...
                            <td class="py-3 text-right">
                                <span
                                    class="text-sm font-medium {{ $transaction['type'] === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
